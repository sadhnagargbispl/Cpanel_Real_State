...
[Rectimestamp] [datetime] DEFAULT (getdate()) NOT NULL)";
            ...
            try
            {
                i = SqlHelper.ExecuteNonQuery(constr,
    ...
    public static bool panverifyC(string txtpan)
    {
        try
        {
            ...
            string str12 = @"select count(panno) as cnt 
                         from KycVerify as a
                         inner join m_membermaster as b 
                ...
                new SqlConnection(HttpContext.Current.Application["Connect"].ToString()))
            {
                ...
                da.SelectCommand.Parameters.AddWithValue("@panno", txtpan);
    ...
    private bool GetKycPerStatusCheck()
    {
        try
        {
            ...
            Ds12 = SqlHelper.ExecuteDataset(constr1, CommandType.Text, str12);
    ...
    private void Fill_State()
    {
        try
        {
            ...
            string str = Obj.Isostart +
            "Select StateCode, StateName from " + Obj.dBName + "..M_STateDivMaster " +
            "Where ActiveStatus='Y' and RowStatus='Y' Order by StateCode"
            + Obj.IsoEnd;
    ...
    protected void BtnIdentity_Click(object sender, EventArgs e)
    {
        ...
        {
            // Session check
            //if (Session["Sessncheck"] == null ||
            //    Session["Sessncheck"].ToString().ToUpper() !=
            ...
            if (Pankyc.HasFile)
            {
                ...
                if (strextension.ToUpper() == ".JPG" || strextension.ToUpper() == ".JPEG" || strextension.ToUpper() == ".PNG")
                {
                    ...
                    else
                    {
                        ...
                        //Pankyc.PostedFile.SaveAs(Server.MapPath("images/UploadImage/") + FlPan);
                ...
                if (!string.IsNullOrWhiteSpace(Txtbank.Text))
                {
                    ...
                    if (Dt.Rows.Count == 0)
                    {
                        ...
                        if (i > 0)
                        {
                            ...
                            dt1 = SqlHelper.ExecuteDataset(constr1, CommandType.Text, Q1).Tables[0];
            ...
            DataTable dtCompare = SqlHelper.ExecuteDataset(constr1, CommandType.Text, str).Tables[0];
            ...
            //{
            //    scrname = "<SCRIPT language='javascript'>alert('Choose Account Type.');</SCRIPT>";
            //    RegisterStartupScript("MyAlert", scrname);
            //    return;
            //}
            ...
            if (isOk1 > 0)
            {
                ...
                if (j != 0)
                {
                    //scrname = "<SCRIPT language='javascript'>alert('KYC Upload successfully. ');</SCRIPT>";
                    string url = "kyc.aspx";
                    string script = "window.onload = function(){ alert('";
                    script += "KYC Upload successfully";
                    script += "');";
